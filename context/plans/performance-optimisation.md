...
Partially implemented. The profiling system (per-system timing, auto-exit, JSON + Markdown reports) is live behind `--features profiling`. PPO performance optimisations completed: flat weight storage, pre-allocated scratch buffers, batched forward/backward, iterator-based loops, swap instead of clone, Adam bias correction precomputation, asymmetric actor-critic (actor 2x64, critic 2x128), amortised PPO updates at 64 samples/tick. Car count bumped to 8. Result: 426 stutters reduced to 2, mean frame time from 17.3ms to 9.0ms. The live TUI viewer remains idea-stage. Raycasting, rendering, and analytics optimisations remain future work.
